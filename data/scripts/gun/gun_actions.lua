actions =
{
	...
	},
	{
		...
		action 		= function()
			...
			c.fire_rate_wait = c.fire_rate_wait + 30
			c.child_speed_multiplier = c.child_speed_multiplier * 0.75
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 0.75, 0), 20)
			shot_effects.recoil_knockback = 60.0
	...
	},
	{
		...
		action 		= function()
			...
			c.speed_multiplier = math.min(math.max(c.speed_multiplier, 0), 20)
	...
	]]--
	{
		...
		action 		= function()
			...
			c.spread_degrees = c.spread_degrees + 4.0
			c.fire_rate_wait = c.fire_rate_wait + 10
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 1.1, 0), 20)
		end,
	},
	{
		id          = "DARKFLAME",
	...
	},
	{
		...
		action 		= function()
			...
			local egg_name = "egg_" .. types[rnd] .. ".xml"
	...
	},
	{
		...
		action 		= function()
			...
			local c = c
			c.fire_rate_wait = c.fire_rate_wait + 10
			c.speed_multiplier = math.min(math.max(c.speed_multiplier - 2, 0), 20)
	...
	},
	{
		...
		action 		= function()
			...
			c.fire_rate_wait = 50
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 0.5, 0), 20)
			c.material = "fire"
	...
	},]]--
	{
		...
		action 		= function()
			...
			local rnd = Random(1, #types)
			local firework_name = "firework_" .. types[rnd] .. ".xml"
	...
	},
	{
		...
		action 		= function()
			...
			local damagecomp = EntityGetComponent( entity_id, "DamageModelComponent" )
			
			if ( damagecomp ~= nil ) and ( #damagecomp > 0 ) then
				for a,b in ipairs( damagecomp ) do
	...
	},
	{
		...
		action 		= function()
			...
			local multiplier = Random( -3, 4 ) * Random( 0, 2 )
			local c = c
			local result = c.damage_projectile_add + 0.4 * multiplier
	...
	]]--
	{
		...
		action 		= function()
			local c = c
			local shot_effects = shot_effects
			...
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 0.3, 0), 20)
			shot_effects.recoil_knockback = shot_effects.recoil_knockback + 50.0
			draw_actions( 1, true )
		end,
	},
	{
		...
		action 		= function()
			...
			c.explosion_radius = math.max(c.explosion_radius - 10, 0)
			c.fire_rate_wait = c.fire_rate_wait - 3
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 0.75, 0), 20)
			c.spread_degrees = c.spread_degrees - 6
	...
	},
	{
		...
		action 		= function()
			local c = c
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 2.5, 0), 20)
			draw_actions( 1, true )
		end,
	},
	{
		...
		action 		= function()
			local c = c
			c.fire_rate_wait = c.fire_rate_wait + 8
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 0.32, 0), 20)
	...
	},
	{
		...
		action 		= function()
			c.fire_rate_wait = c.fire_rate_wait - 8
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 1.68, 0), 20)
	...
	},
	{
		...
		action 		= function()
			...
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 1.2, 0), 20)
			c.fire_rate_wait = c.fire_rate_wait - 10
			draw_actions( 1, true )
		end,
	},
	...
	},
	{
		...
		type 		= ACTION_TYPE_MODIFIER,
		spawn_level                       = "1,2,3", -- FIREBALL_RAY_ENEMY
	...
	},
	{
		...
		type 		= ACTION_TYPE_MODIFIER,
		spawn_level                       = "0,1,3,4", -- FIREBALL_RAY_ENEMY
	...
	},
	{
		...
		action 		= function()
			...
			c.damage_projectile_add = c.damage_projectile_add - 0.2
			c.explosion_radius = math.max(c.explosion_radius - 5, 0)
			c.spread_degrees = c.spread_degrees + 10.0
			draw_actions( 1, true )
		end,
	},
	...
	},
	{
		...
		action 		= function()
			local c = c
			c.speed_multiplier = math.min(math.max(c.speed_multiplier * 0.4, 0), 20)
	...
	},
	{
		...
		action 		= function( recursion_level, iteration )
			...
			local data = discarded[1] or hand[1] or deck[1]
			local rec = check_recursion( data, recursion_level )
	...
	},
	{
		...
		action 		= function( recursion_level, iteration )
			...
			local data = deck[#deck] or hand[#hand]
			local rec = check_recursion( data, recursion_level )
	...
	},
	{
		...
		action 		= function( recursion_level, iteration )
			c.fire_rate_wait = c.fire_rate_wait + 50
			local function act(data, rec)
				dont_draw_actions = true
				data.action(rec)
				dont_draw_actions = false
			end
			for i, cdeck in ipairs({discarded or {}, hand or {}, deck or {}}) do
				for i, data in ipairs(cdeck) do
					local rec = check_recursion(data, recursion_level)
					if data ~= nil and ((rec > -1 and data.id ~= "RESET")
					or (cdeck == hand and not data.recursive)) then
						act(data, rec)
					end
				end
			end
	...
	},
	{
		...
		action 		= function()
			...
			for i,_deck in ipairs({hand, deck}) do
				for i,v in ipairs( _deck ) do
	...
	},
	{
		...
		action 		= function()
			...
			c.fire_rate_wait = c.fire_rate_wait - 8
			c.screenshake = math.max(c.screenshake - 2.5, 0)
			draw_actions( 1, true )
		end,
	},
	{
