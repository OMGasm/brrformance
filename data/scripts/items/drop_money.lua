...
function do_money_drop( amount_multiplier, trick_kill )
	...
	---
	for k,v in ipairs({{50000, 12}, {10000, 12}, {1000, 20}, {200, 12}, {50, 9}, {10, 6}}) do
		local monmons, idk = table.unpack(v)
		local eid = load_gold_entity(string.format("%s%s.xml", gold_entity, monmons), x, y-8, remove_timer)
		money = money - monmons
		table.insert(entity_list, {eid, idk})
	end
